![docker image](https://github.com/nomad-hzb/beamline-image/actions/workflows/docker-publish.yml/badge.svg)
...
2. Clone the repository or download the repository as a zip file.
    ...
    ```sh
    git clone https://github.com/nomad-hzb/beamline-image.git
    cd beamline-image
    ```
    ...
    ```sh
    curl-L -o beamline-image.zip "https://github.com/nomad-hzb/beamline-image/archive/main.zip"
    unzip beamline-image.zip
    cd beamline-image
    ```
...
your `docker-compose.yaml` with `ghcr.io/nomad-hzb/beamline-image:main` for the services
...
```
docker pull ghcr.io/nomad-hzb/beamline-image/jupyter:main
```
...
_I get an_ `Error response from daemon: Head "https://ghcr.io/v2/nomad-hzb/beamline-image/manifests/main": unauthorized`
